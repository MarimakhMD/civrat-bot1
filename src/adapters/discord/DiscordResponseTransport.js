...
class DiscordResponseTransport {
  ...
  async replyEmbed({ embed, components = [], ephemeral = true }) { const builder = new EmbedBuilder(); if (embed.title) builder.setTitle(embed.title); if (embed.description) builder.setDescription(embed.description); if (embed.color) builder.setColor(embed.color); if (embed.image) builder.setImage(embed.image); const data = { embeds: [builder], components: toActionRows(components), ephemeral }; if (this.interaction.replied || this.interaction.deferred) return this.interaction.followUp(data); return this.interaction.reply(data); }
