...
class DiscordInteractionAdapter {
  ...
  normalize(interaction) {
    ...
    if (!kind) return null;
    return { kind, name: interaction.commandName || null, customId: interaction.customId || null, values: interaction.values || [], guildId: interaction.guildId || null, userId: interaction.user?.id || null, member: createDiscordMemberCapability(interaction.member, interaction.guild?.ownerId), discordMember: interaction.member, transport: new DiscordResponseTransport(interaction) };
  }
}
