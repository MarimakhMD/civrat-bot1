...
// Persists the administrator template choice and re-renders the section view.
...
async function selectWelcomeTemplate(context) {
  ...
  await context.envelope.transport.update({ view: settingsView({ t: context.t, config }) });
  return config;
}
