...
async function updateWelcomeSettings(context, updates) { const config = await context.settings.update(context.guildId, updates); const view = settingsView({ t: context.t, config }); view.content = `${welcomeUpdatedMessage(context.t, config)}\n${view.content}`; await context.envelope.transport.update({ view }); return config; }
