...
function redactString(value) {
  ...
  output = output.replace(
    /\b(discord[_-]?token|owner[_-]?panel[_-]?master[_-]?code|owner[_-]?transfer[_-]?code|recovery[_-]?(?:master[_-]?)?code|smtp[_-]?password|supabase[_-]?service[_-]?role[_-]?key|mongo[_-]?uri|api[_-]?key|client[_-]?secret|password|passwd|secret)\b(\s*[:=]\s*)["']?[^\s,;"']+["']?/gi,
    `$1$2${REDACTED}`,
  );
  output = output.replace(
