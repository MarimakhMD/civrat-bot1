...
test("modern Supabase keys and endpoints are redacted in strings and metadata", () => {
  const secretKey = ["sb", "secret", privateMarker("modern-service")].join("_");
  const publishableKey = ["sb", "publishable", privateMarker("modern-public")].join("_");
  const hostname = ["project-marker", "supabase", "co"].join(".");
  const backendUrl = ["https://", hostname, "/rest/v1/config"].join("");

  const output = captureLogs(() => logger.error(
    `backend request failed at ${backendUrl}; credential=${secretKey}`,
    {
  ...
  assertHidden(output, [secretKey, publishableKey, backendUrl, hostname]);
  assert.match(output, /\[REDACTED\]/);
  assert.match(output, /"guildId":"1320817768962064384"/);
  assert.match(output, /"discordCode":50001/);
});
